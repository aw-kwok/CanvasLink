 ...
 * 
 * @async
 * @param {Object} settingsParams - settingsParams object with apiUrlInput, apiKeyInput, and semesterInput
 * @returns {Object} settings - Saved settings
 */
async function saveSettings({ apiUrlInput, apiKeyInput, semesterInput }) {
    try {
        ...
        if (!res.ok) throw new Error("API URL invalid");
...
/**
 * Syncs events from a user's Canvas calendar to their Google calendar
 * 
 * @async
 */
 ...
 */
async function buildCourses(settingsParams) {
    if (debug) console.log("building courses");

    try {
        const courses = await getCourses(settingsParams);
        if (debug) console.log(courses);
 ...
 */
async function getCourses(settingsParams) {
    try {
        const settings = await saveSettings(settingsParams);
...
export { syncEvents, loadCanvasEvents, buildCourses, saveSettings, getCourses, updateCourseColor, loadCourseBoxes };